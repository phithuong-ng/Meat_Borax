...
# --- 2. TẢI DỮ LIỆU ---
print(">>> Đang chạy kiểm định Full Report...")
try:
...
# --- 5. XUẤT BÁO CÁO ---
print("\n" + "="*60)
print("          BÁO CÁO KIỂM ĐỊNH HỆ THỐNG (FINAL REPORT)")
print("="*60)
...
if len(wrong) > 0:
    print(f"{'Tên File':<20} | {'Thực tế':<10} | {'Máy đoán':<10} | {'Nguyên nhân dự đoán'}")
    print("-" * 70)
    for idx, row in wrong.iterrows():
        thuc = "SẠCH" if row['Label']==0 else "BẨN"
        doan = "SẠCH" if row['Predicted']==0 else "BẨN"
        ly_do = "Nồng độ quá thấp / Nhiễu" if row['Label']==1 else "Mẫu sạch bị biến chất / Nhiễu"
        print(f"{row['FileName']:<20} | {thuc:<10} | {doan:<10} | {ly_do}")
    print("-" * 70)
else:
    print(">>> TUYỆT VỜI! Không có mẫu nào bị nhận diện sai.")
    print(">>> Mô hình đạt độ tin cậy tuyệt đối trên tập dữ liệu này.")